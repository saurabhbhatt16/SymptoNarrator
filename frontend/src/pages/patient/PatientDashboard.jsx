...
import { analyzeSymptomsApi, predictDiseaseApi } from '../../services/ai.service'
import { getPatientProfileApi, updatePatientProfileApi } from '../../services/patient.service'
...
import { createMyReportApi, getMyReportsApi } from '../../services/report.service'
import {
...
function buildAppointmentNotifications(appointments, existingNotifications) {
  const existingKeys = new Set((existingNotifications || []).map((item) => item.key))
  const additions = []
  ...
  appointments.forEach((appointment) => {
    ...
    if (['pending', 'accepted'].includes(appointment.status)) {
      ...
      if (minutesUntilAppointment > 0 && minutesUntilAppointment <= 30 && !existingKeys.has(reminderKey)) {
        additions.push({
          key: reminderKey,
...
function PatientDashboard() {
  ...
  const loadDashboard = async () => {
    ...
    try {
      ...
      const generatedNotifications = buildAppointmentNotifications(nextAppointments, existingNotifications)
      const mergedNotifications = [...generatedNotifications, ...existingNotifications]
  ...
  const handleToggleNotifications = () => {
    setNotificationOpen((current) => {
      const next = !current
      if (!current) {
        const markedAsRead = notifications.map((item) => ({ ...item, isRead: true }))
  ...
  const upcomingAppointments = useMemo(() => {
    const now = Date.now()
    return appointments.filter((item) => isAppointmentUpcomingForView(item, now))
  }, [appointments])

  const totalAppointments = useMemo(() => {
    const now = Date.now()
    return appointments.filter((item) => isAppointmentCompletedForView(item, now))
  }, [appointments])

  const stats = useMemo(() => {
    const lastPredictionLabel = lastPrediction?.diagnosis?.diseaseName || lastPrediction?.disease || 'No prediction yet'
    ...
    return {
      ...
      upcomingDetails: upcomingAppointments.map(
        (item) => `${item.doctor?.fullName || 'Doctor'} - ${new Date(item.date).toLocaleString()}`,
      ),
  ...
  const symptomHistory = useMemo(() => {
    return [...new Set(predictions.flatMap((item) => extractSymptomsFromText(item?.form?.symptoms)))].slice(0, 20)
  }, [predictions])
  ...
  const confirmCancelAppointment = async () => {
    ...
    try {
      ...
      toast.error(requestError?.response?.data?.message || 'Failed to cancel appointment')
    } finally {
  ...
  const handleRescheduleAppointment = async (id) => {
    ...
    try {
      ...
      toast.error(requestError?.response?.data?.message || 'Failed to reschedule appointment')
    } finally {
      setActionLoadingId(null)
    }
  }
  ...
  const saveToHistory = (reportData) => {
    setPredictions((current) => {
      const deduped = current.filter((item) => item.generatedAt !== reportData.generatedAt)
  ...
  const downloadPDF = () => {
    ...
    doc.setFontSize(11)
    doc.text(`Date: ${reportDate.toLocaleDateString()} ${reportDate.toLocaleTimeString()}`, 10, y)
    y += 8
    ...
    y += 5
    doc.text(`Previous Illness: ${report.form?.previousIllness || '--'}`, 10, y)
    y += 8
    ...
    y += 5
    doc.text(`Specialist Doctor: ${report.diagnosis?.specialistDoctor || '--'}`, 10, y)
    y += 5
    ...
    y += 6
    doc.setFontSize(11)
    doc.text(medicines.length > 0 ? medicines.join(', ') : 'No suggested medicines available.', 10, y)
    y += 8
    ...
    y += 6
    doc.setFontSize(11)
    const wrappedSummary = doc.splitTextToSize(report.summary || 'Summary not available.', 180)
  ...
  const generateReport = async () => {
    ...
    try {
      const correctedSymptoms = correctSymptoms(symptoms)
      ...
      try {
        ...
        reportData = {
          ...
          diagnosis: {
            diseaseName: analysis?.diagnosis?.disease_name || 'General health concern',
            ...
            specialistDoctor: analysis?.specialist_required || 'General Physician',
          ...
          treatmentPlan: Array.isArray(analysis?.treatment?.plan) ? analysis.treatment.plan : [],
          medicines: Array.isArray(analysis?.treatment?.medicines) ? analysis.treatment.medicines : [],
        ...
        let prediction = {}
        try {
          const predictionResponse = await predictDiseaseApi({ symptoms: correctedSymptoms })
        ...
        const fallbackSummary =
          `${fullName} (${patientAge}, ${String(patientGender).toLowerCase()}) has reported ${correctedSymptoms} for ${parsedDuration} day(s). ` +
    ...
    <div className="min-h-screen overflow-x-hidden bg-slate-100">
      <main className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6">
        <div className="flex items-start justify-between gap-4">
          <h1 className="text-3xl font-bold text-slate-900">Welcome, {fullName}</h1>
          <div ref={notificationRef} className="relative shrink-0">
            ...
            {notificationOpen ? (
              <div className="absolute right-0 z-40 mt-2 w-[min(90vw,22rem)] rounded-xl border border-slate-200 bg-white p-3 shadow-xl">
                <p className="px-1 text-sm font-semibold text-slate-800">Notifications</p>
        ...
        <section className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-[300px_minmax(0,1fr)_280px] lg:grid-cols-2">
          ...
          <div className="space-y-6">
            <AppointmentDashboardCard appointments={appointments} loading={loading} />
            <SummaryCards
              stats={stats}
              onCardClick={(card) => {
      ...
      {summaryModal ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/45 px-4">
          <div className="w-full max-w-lg rounded-xl bg-white p-5 shadow-2xl">
            <h3 className="text-lg font-semibold text-slate-900">{summaryModal.title}</h3>
              ...
              <div className="mt-4 max-h-72 space-y-3 overflow-auto">
                {upcomingAppointments.length > 0 ? (
                  upcomingAppointments.map((appointment) => {
                    ...
                    const nextDateValue = rescheduleValues[appointment.id] || ''
                    const statusLabel =
                      appointment.status === 'accepted' ? 'Accepted by doctor' : 'Pending appointment'
                    ...
                    return (
                      <div key={appointment.id} className="rounded-lg border border-slate-200 bg-slate-50 p-3">
                        ...
                        <p className="text-sm font-medium text-slate-800">{appointment.doctor?.fullName || 'Doctor'}</p>
                        <p className="mt-1 text-sm text-slate-700">{new Date(appointment.date).toLocaleString()}</p>
                          ...
                          {canReschedule ? (
                            <>
                              <button
                                type="button"
                                onClick={() =>
                                  setRescheduleOpenId((current) => (current === appointment.id ? null : appointment.id))
                                }
                                disabled={actionLoadingId === appointment.id}
                              ...
                              {rescheduleOpenId === appointment.id ? (
                                <div className="mt-1 flex flex-col gap-2 rounded-lg border border-slate-200 bg-white p-2">
                                  <label className="text-xs font-medium text-slate-600">Select date and time</label>
                                  <input
                                    type="datetime-local"
                                    value={nextDateValue}
                                    onChange={(event) =>
                  ...
                  <p className="rounded-lg bg-slate-50 px-3 py-2 text-sm text-slate-700">No upcoming appointments.</p>
                )}
              </div>
            ) : (
              <div className="mt-4 max-h-72 space-y-2 overflow-auto">
                {(summaryModal.details || []).map((line) => (
                  <p key={line} className="rounded-lg bg-slate-50 px-3 py-2 text-sm text-slate-700">
                    {line}
                  </p>
                ))}
              </div>
      ...
      {cancelConfirmId ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/45 px-4">
          <div className="w-full max-w-sm rounded-xl bg-slate-100 p-5 shadow-2xl">
            <h3 className="text-base font-semibold text-slate-900">Confirm Cancel Appointment</h3>
            <p className="mt-2 text-sm text-slate-700">Are you sure you want to cancel this appointment?</p>
