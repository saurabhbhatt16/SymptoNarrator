...
function stripDisclaimer(text = "") {
  ...
  if (!value) return { summary: "", disclaimer: NEW_DISCLAIMER };

  let summary = value;
  let disclaimer = NEW_DISCLAIMER;

  if (summary.includes(OLD_DISCLAIMER)) {
    summary = summary.replace(OLD_DISCLAIMER, "").trim();
  }

  if (summary.includes(NEW_DISCLAIMER)) {
    summary = summary.replace(NEW_DISCLAIMER, "").trim();
  }
...
function normalizeSingleReport(report = {}) {
  ...
  const { summary, disclaimer } = stripDisclaimer(report?.summary || "");
  ...
  return {
    ...
    diagnosis: {
      ...diagnosis,
      diseaseName:
        report?.diagnosis?.diseaseName || diagnosis?.disease_name || "--",
      specialistDoctor:
        report?.diagnosis?.specialistDoctor ||
    ...
    summary,
    translatedSummary:
      report?.translatedSummary || report?.translated_summary || summary,
    disclaimer,
  };
}
...
function normalizeReportsPayload(payload) {
  const data = Array.isArray(payload?.data)
    ? payload.data.map((item) => normalizeSingleReport(item))
    : [];
  return { data };
}
...
export async function createMyReportApi(report) {
  ...
  const saved = response?.data?.data
    ? normalizeSingleReport(response.data.data)
    : null;
